...
function PLUGIN:ItemPressedRightShift(item, ply, ent)
    if (item.uniqueID == "cards_deck_cards") then
        item.functions.PickUp2.OnRun(item, ply)
    end
end

function PLUGIN:PlayerButtonDown(ply, button)
    if ( button ==  MOUSE_RIGHT  ) then
        local ThingSeen = ply:GetEyeTrace()
        if( ThingSeen.Entity:GetClass() == "ix_item") then
            local HelixItem = ThingSeen.Entity:GetItemTable()
            PLUGIN:ItemPressedRightShift(HelixItem, ply, ThingSeen.Entity)
        end
    end
end
