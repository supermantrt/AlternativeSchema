...
-- messy but idc.
function PLUGIN:SearchLootContainer(ent, ply, loot, lootpos)
                ...
                ply:SetAction("Searching...", LootLength, function()
                    ...
                    for i = 1, lootAmount do
                        ...
                        if (General == true) then
                            local RandomPos = table.Random(lootpos)
                            local posspawn = ent:GetAttachment(RandomPos.id).Pos
                            local posang = ent:GetAttachment(RandomPos.id).Ang
                            randomLootItem = table.Random(PLUGIN.randomLoot.general)
                            print(posspawn,posang)
--                            randomLootItem:Spawn(posspawn, ang)
                            ix.item.Spawn(randomLootItem,posspawn, IDGrab(randomLootItem),posang)
--                            ply:ChatNotify("You have gained "..ix.item.Get(randomLootItem):GetName()..".")
                            --ply:GetCharacter():GetInventory():Add(randomLootItem)
                        end
                        if (Horror == true) then
