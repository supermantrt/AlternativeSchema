		...
		OnDisplay = function(self, container, payload)
			...
			local function clearTraits()
				...
				for _, v in pairs(bgTraits) do
					...
					payload.traits[_] = nil
					bgTraits = {}
					
					for x , y in pairs(ix.traits.list) do
						if(_ == x) then
							y.BackstoryAdded = false
						end
					end
				end
...
--			descLabel:SetText( "" )
			...
			function bgdropdown:OnSelect( index, value, data )
				...
				for k, v in pairs( data.traits ) do
					...
					addTrait(v)
					for x , y in pairs( traits ) do
						if(v == x) then
							y.BackstoryAdded = true 
						end
					end
...
--	PrintTable(ix.backgrounds.list)
end