...
function ix.safebox.Restore(client, callback)
	...
	if(client:GetNetVar("NewSafeBoxPurchase" , false ) == true) then
		ix.inventory.New(character:GetID(), "safebox1", function(inv)
			character:SetData("safeboxID", inv:GetID())
		end)
		client:SetNetVar("NewSafeBoxPurchase" , false )
		client:SetNetVar("SafeBoxPurchaseCheck" , true )
	end


	if(client:GetNetVar("NewSafeBoxPurchase2" , false ) == true) then
		ix.inventory.New(character:GetID(), "safebox2", function(inv)
			character:SetData("safeboxID", inv:GetID())
		end)
		client:SetNetVar("NewSafeBoxPurchase2" , false )
	end
