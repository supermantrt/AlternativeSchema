...
ITEM.gasmask = false
ITEM.repairitem = "scrap"
ITEM.repairitemamount = 3
...
ITEM.functions.repair = {
	...
	OnRun = function(item)
		...
		local amount = 0
		local uniqueID = item.repairitem
		local Anounceamountneeded = item.repairamount
		...
		if(client:Team() == FACTION_MEC) then
		    ...
		    if (character:GetAttribute("int") >= 5) then
				for _, itemTable in pairs(inventory:GetItemsByUniqueID(uniqueID)) do
					...
					if (itemTable.base && uniqueID) then
						if (itemTable.base == 'base_stackable') then
							if(amount == item.repairitemamount) then
								break
							end

							if ( itemTable:GetData('stacks', 1) > item.repairitemamount - amount ) then
								itemTable:SetData('stacks', itemTable:GetData('stacks', 1) - (item.repairitemamount - amount))
								amount = item.repairitemamount
								break
							end

							if ( itemTable:GetData('stacks', 1) == item.repairitemamount - amount ) then
								amount = amount + itemTable:GetData('stacks', 1)
								itemTable:Remove()
								break
							end
				...
				elseif( amount != 0) then
					local amounttaken =  amount - 1
					local x , y = inventory:FindEmptySlot(1,1)


					inventory:FindEmptySlot(x,y)
					inventory:Add(item.repairitem, 1 , nil ,x,y)

					if(amounttaken > 0 and x != nil ) then
						for _, itemTable in pairs(inventory:GetItemsByUniqueID(uniqueID)) do
							...
							if (itemTable.base && uniqueID) then
								if (itemTable.base == 'base_stackable') then
									...
									if ( itemTable:GetData('stacks', 1) < itemTable.maxStacks ) then
										local maxStack = itemTable.maxStacks
										local AmountReturned = (itemTable:GetData('stacks', 1) + amounttaken) - maxStack

										if(AmountReturned <= 0) then
											itemTable:SetData('stacks', itemTable:GetData('stacks', 1) + amounttaken)
											break
										else
											x , y = inventory:FindEmptySlot(1,1)
											inventory:Add(item.repairitem, 1 , nil ,x,y)
			        ...
			        client:Notify("You need ".. item.repairitemamount - amount .." more ".. item.repairitem .." to repair this")
				else
					client:Notify("You need ".. item.repairitemamount .." ".. item.repairitem .." to repair this")
				end
		    else
...
function PLUGIN:InventoryItemAdded(oldInv, inventory, item)
	local ply = inventory:GetOwner()
	if(ply:GetNetVar("ArmourFlip", false) == true) then
		item.Durability = item:SetData("durabilityvar", ply:GetNetVar("OldArmourValue", item.Durability))
		ply:SetNetVar("ArmourFlip", false)
	end
end
]]
