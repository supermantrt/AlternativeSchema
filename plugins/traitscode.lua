...
-- [Trait Function Activate KeypPressed]
...
function PLUGIN:CanPlayerTradeWithVendor(client, entity, uniqueID, isSellingToVendor)
	if(client:GetNetVar("hasnegotatitor", false) == true) then
		...
		client:SetNetVar("negotatitormoney", client:GetCharacter():GetMoney())
	end
end
...
function PLUGIN:CharacterVendorTraded(client, entity, uniqueID, isSellingToVendor)
	if(client:GetNetVar("hasnegotatitor", false) == true) then
		if(isSellingToVendor == true) then
			...
			local ItemPricing = client:GetCharacter():GetMoney() - client:GetNetVar("negotatitormoney",client:GetCharacter():GetMoney())
			local ItemPricingBoost = math.floor(ItemPricing * math.Rand(0,1))

			client:GetCharacter():SetMoney(client:GetCharacter():GetMoney() + ItemPricingBoost)
...
function PLUGIN:Pickpocket(ply)
	...
	if SERVER then
		...
		if(DelayPickpocketSupermantrt == nil) then
			DelayPickpocketSupermantrt = Time.Now
		end
		...
		if( DelayPickpocketSupermantrt > Time.Now ) then
		...
		if(math.random(0,101) < 20 ) then
			...
			DelayPickpocketSupermantrt = Time.Now + Time.Seconds( 30 )
			return

		end

		if(target:IsPlayer() == true and DelayPickpocketSupermantrt <  Time.Now + Time.Seconds( 30 )) then
...
function PLUGIN:CanTransferItem(item, currentInv, oldInv)
	if SERVER then
		...
		if(ply:GetNetVar("hasthievinghands", false) == true) then
			if(ply:GetNetVar("attemptpickpocket", false) == true) then
